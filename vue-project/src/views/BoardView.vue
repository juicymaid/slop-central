<template>
    <div class="max-w-[1200px] mx-auto px-6 mb-12 transition-colors duration-200">
        <h1 class="text-4xl font-serif font-bold italic mb-4 mt-16 text-[#FAF8F5] drop-shadow-md">{{board?.name}}</h1>
        <h1 class="text-lg font-sans font-medium mb-4 text-[#C9A84C]">{{board?.tags}}</h1>
        <h2 class="text-[#FAF8F5]/60 font-mono text-sm uppercase tracking-wider">{{ board?.images.length }} pins</h2>
    </div>
    <div class="px-6  mx-auto">
        <ImageMasonry :pins="board?.images" />
    </div>
    <h1 class="text-3xl font-serif font-bold italic mb-8 mt-24 text-center text-[#FAF8F5] drop-shadow-md">More Ideas</h1>
    <div class="px-6  mx-auto">
