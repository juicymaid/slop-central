<template>
    <div class="max-w-[1400px] mx-auto px-6 mb-12">
        <h2 class="text-3xl font-serif font-bold italic mb-8 mt-12 flex items-center text-[#FAF8F5] drop-shadow-md">
            Boards
            <button @click="showModal = !showModal" class="magnetic-button ml-4 p-2 rounded-full hover:bg-[#2A2A35] transition-colors border border-[#2A2A35] bg-[#1A1A24]">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 group cursor-pointer mb-16">
            <div v-for="board in boards" :key="board.id" class="bg-[#1A1A24] rounded-[2rem] shadow-lg hover:shadow-[0_4px_25px_rgba(0,0,0,0.5)] transition-all duration-300 overflow-hidden border border-[#2A2A35] group/board">
                <RouterLink :to="'/board/' + board.id">
                    <div class="flex">
                        <img v-if="board.cover_images.length >= 1" :src="ImageSrc(board.cover_images[0])" alt="Board cover" class="w-48 rounded-tl-[2rem] h-48 object-cover group-hover/board:scale-105 transition-transform duration-500" />
                        <div v-else class="w-48 h-48 bg-[#0D0D12] rounded-tl-[2rem]"></div>
                        <div class="h-48 flex flex-col w-full overflow-hidden">
                            <img v-if="board.cover_images.length >= 2" :src="ImageSrc(board.cover_images[1])" alt="Board cover" class="w-full h-full object-cover group-hover/board:scale-105 transition-transform duration-500" />
                            <img v-if="board.cover_images.length >= 3" :src="ImageSrc(board.cover_images[2])" alt="Board cover" class="w-full rounded-tr-[2rem] h-full object-cover group-hover/board:scale-105 transition-transform duration-500" />
                        </div>
                    </div>
