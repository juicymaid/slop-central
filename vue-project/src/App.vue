...
<template>
  ...
  <div class="min-h-screen transition-colors duration-300 bg-[#0D0D12]">
    ...
    <nav
      v-if="!hideNav"
      id="app-header" class="app-header fixed top-4 left-1/2 -translate-x-1/2 transition-colors duration-200 bg-[#0D0D12]/60 backdrop-blur-xl border border-[#2A2A35] rounded-full shadow-lg z-40 px-6 py-3"
      ...
      <div class="flex items-center justify-between w-full mx-auto">
        ...
        <RouterLink to="/" class="flex items-center group">
          <svg class="w-8 h-8 text-[#C9A84C] transition-transform duration-300 group-hover:scale-105" fill="currentColor" viewBox="0 0 24 24">
            <path
          ...
          <span class="hidden md:inline-block ml-3 font-semibold font-sans text-lg tracking-tight text-[#FAF8F5]">Slop Central</span>
        </RouterLink>
        ...
        <div class="flex-1 mx-2 md:mx-6 max-w-2xl">
          <div class="relative group">
            ...
            <div v-else-if="showSuggestions"
              ...
              <div class="px-4 py-2">
                <h3 class="text-xs uppercase tracking-wider font-semibold text-[#FAF8F5]/50 font-sans mb-3">Popular Tags</h3>
                ...
                <div class="grid gap-3" style="grid-template-columns: repeat(auto-fill, minmax(180px, 1fr));">
                  ...
                  <div v-for="tag in tags.top_tags.slice(0, 8)" :key="tag.tag"
                    ...
                    <div class="w-12 h-12 flex-shrink-0 overflow-hidden rounded-lg relative">
                      <div class="absolute inset-0 bg-[#0D0D12]/20 group-hover:bg-transparent transition-colors z-10"></div>
                      <img :src="ImageSrc(tag.cover_image)" class="w-full h-full object-cover grayscale opacity-80 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300">
                    </div>

                    <div class="overflow-hidden">
                      <p class="text-sm font-semibold text-[#FAF8F5] font-sans truncate group-hover:text-[#C9A84C] transition-colors">
                        {{ tag.tag }}
                      </p>
                ...
                <h3 class="text-xs uppercase tracking-wider font-semibold text-[#FAF8F5]/50 font-sans mt-6 mb-3">Ideas for you</h3>
                <div class="grid gap-3" style="grid-template-columns: repeat(auto-fill, minmax(180px, 1fr));">
        ...
        <button @click="toggleDarkMode" class="md:hidden p-2 text-[#FAF8F5]/80 hover:text-[#FAF8F5] transition-colors cursor-pointer mr-1" aria-label="Toggle Theme">
    ...
    <nav v-if="!hideNav && isMobile" class="fixed bottom-0 left-0 right-0 bg-[#0D0D12]/90 backdrop-blur-xl border-t border-[#2A2A35] py-2 px-6 flex items-center justify-between z-40 shadow-lg">
      <RouterLink to="/" class="flex flex-col items-center gap-1 text-[#FAF8F5]/70 hover:text-[#FAF8F5] transition-colors">
      ...
      <button @click="webState.sidebarWidth = webState.sidebarWidth === 0 ? 500 : 0;" class="flex flex-col items-center gap-1 text-[#C9A84C] hover:text-[#FAF8F5] transition-colors cursor-pointer bg-transparent border-none">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <RouterLink to="/posts" class="flex flex-col items-center gap-1 text-[#FAF8F5]/70 hover:text-[#FAF8F5] transition-colors">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012-2h10a2 2 0 012 2v1m2 4a2 2 0 00-2-2v3m2-3V9m0 0l-9 9m9-9h-4m4 0v4" />
        </svg>
        <span class="text-[10px] font-sans">Posts</span>
      </RouterLink>

      <RouterLink to="/scan" class="flex flex-col items-center gap-1 text-[#FAF8F5]/70 hover:text-[#FAF8F5] transition-colors">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 13h6m-3-3v6m-9 1V4a2 2 0 012-2h6l2 2h6a2 2 0 012 2v10a2 2 0 01-2 2H5a2 2 0 01-2-2z" />
        </svg>
        <span class="text-[10px] font-sans">Scan</span>
      </RouterLink>

      <RouterLink to="/profile" class="flex flex-col items-center gap-1 text-[#FAF8F5]/70 hover:text-[#FAF8F5] transition-colors">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
