	...
	<div class="flex flex-col h-full bg-[#0D0D12] text-[#FAF8F5]">
		...
		<div class="flex items-center justify-between p-6 border-b border-[#2A2A35] hidden">
			<div class="flex items-center gap-3">
				...
				<span
					class="text-[10px] px-3 py-1 rounded-full font-mono uppercase tracking-widest"
			...
			<div class="flex items-center gap-2">
				<button @click="clearChat" class="magnetic-button text-[#FAF8F5]/60 hover:text-[#FAF8F5] p-2 rounded-full transition-colors" title="Clear chat">
		...
		<div ref="scrollArea" class="flex-1 min-h-0 overflow-y-auto px-6 py-4 space-y-4">
			...
			>
				...
				<div v-if="m.role !== 'user'" class="flex items-start gap-3 max-w-[85%]">
					<div class="w-8 h-8 rounded-full bg-[#2A2A35] border border-[#C9A84C]/30 flex items-center justify-center text-xs font-serif font-bold italic text-[#C9A84C] shadow-[0_0_10px_rgba(201,168,76,0.1)] flex-shrink-0 mt-1">A</div>
					<!-- Text response -->
					<div v-if="(!m.type || m.type === 'text') && (m.text != '' || m.thinking)" class="bg-[#1A1A24] border border-[#2A2A35] rounded-2xl rounded-tl-sm px-5 py-4 text-sm leading-relaxed text-[#FAF8F5]/90 shadow-sm relative overflow-hidden group">
						...
						<div v-if="m.thinking" class="mb-3">
							...
							>
								...
								<span class="text-xs transition-transform duration-300" :class="m.showThinking ? 'rotate-180 text-[#C9A84C]' : ''">▾</span>
							</button>
							<div
								v-if="m.showThinking"
					...
					<!-- Image response -->
					<div v-else-if="m.type === 'image'" class="bg-[#1A1A24] border border-[#2A2A35] rounded-[2rem] p-2 shadow-lg">
						<img :src="displaySrc(m.url)" alt="tool image" class="max-w-[480px] rounded-[1.5rem]" />
					</div>
					<!-- Search results -->
					<div v-else-if="m.type === 'search'" class="bg-[#1A1A24] border border-[#2A2A35] rounded-[2rem] p-5 w-full shadow-lg">
						<div class="flex flex-col md:flex-row gap-5">
							<div class="md:w-1/3">
								<div class="text-[10px] font-mono text-[#FAF8F5]/40 uppercase tracking-widest mb-2">Search Query</div>
								<div class="text-sm font-serif italic text-[#C9A84C] break-words">{{ m.query }}</div>
							</div>
							<div class="md:w-2/3">
								<div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
									<div v-for="it in (m.items || [])" :key="it.id" class="rounded-xl overflow-hidden border border-[#2A2A35]">
										<img :src="displaySrc(it.url)" class="w-full h-28 object-cover cursor-pointer hover:scale-105 transition-transform duration-500" :alt="it.prompt || ('image ' + it.id)" />
									</div>
								</div>
							</div>
						</div>
					...
					<!-- Status bubble -->
					<div v-else-if="m.type === 'status'" class="bg-[#0D0D12] border border-[#2A2A35] rounded-full px-4 py-2 text-xs font-mono text-[#FAF8F5]/60 shadow-inner">
						{{ m.text }}
					</div>
					<!-- Prompt bubble -->
					<div v-else-if="m.type === 'prompt'" class="bg-[#1A1A24] border border-[#2A2A35] rounded-2xl p-4 text-sm leading-relaxed whitespace-pre-wrap shadow-sm">
						<div class="flex items-start gap-4">
							...
							<button @click="copyPrompt(m.prompt)" class="magnetic-button text-[10px] font-mono uppercase tracking-widest px-3 py-1.5 rounded-lg border border-[#2A2A35] hover:border-[#C9A84C] hover:text-[#C9A84C] text-[#FAF8F5]/60 transition-colors">Copy</button>
						</div>
					</div>
					<!-- CivitAI images bubble -->
					<div v-else-if="m.type === 'civitai_images'" class="bg-[#1A1A24] border border-[#2A2A35] rounded-[2rem] p-5 w-full shadow-lg">
						<div class="flex items-center justify-between gap-3 mb-4">
							<div class="text-[10px] font-mono text-[#FAF8F5]/40 uppercase tracking-widest">CivitAI Images</div>
							<div class="text-[10px] font-mono text-[#FAF8F5]/40 uppercase tracking-widest" v-if="m.filters">
								<span v-if="m.filters.sort">Sort: <span class="text-[#C9A84C]">{{ m.filters.sort }}</span></span>
								<span v-if="m.filters.period" class="ml-3">Period: <span class="text-[#C9A84C]">{{ m.filters.period }}</span></span>
							</div>
						</div>
						<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
							<div v-for="it in (m.items || [])" :key="it.id" class="bg-[#0D0D12] border border-[#2A2A35] rounded-xl overflow-hidden group hover:border-[#C9A84C]/40 transition-colors">
								<div class="aspect-[4/5] bg-[#1A1A24] overflow-hidden">
									<img :src="displaySrc(it.url)" class="object-cover w-full h-full group-hover:scale-105 transition-transform duration-500" :alt="it.prompt || ('image ' + it.id)" />
								</div>
								<div class="p-3">
									<div class="text-[10px] font-mono text-[#FAF8F5]/40 uppercase tracking-widest mb-1">Prompt</div>
									<div class="text-[12px] text-[#FAF8F5]/80 line-clamp-3">{{ it.prompt || 'Prompt unavailable' }}</div>
									<div class="mt-2 flex flex-wrap gap-2 text-[10px] font-mono text-[#FAF8F5]/40 uppercase tracking-widest">
					...
					<div v-else-if="m.type === 'civitai'" class="bg-[#1A1A24] border border-[#2A2A35] rounded-[2rem] p-5 w-full shadow-lg">
						<div class="text-[10px] font-mono text-[#FAF8F5]/40 uppercase tracking-widest mb-4">CivitAI Search: <span class="text-[#C9A84C]">{{ m.query }}</span></div>
						<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
							<div v-for="model in m.items" :key="model.id" class="bg-[#0D0D12] border border-[#2A2A35] rounded-xl p-3 flex flex-col group hover:border-[#C9A84C]/50 transition-colors">
								<div v-if="model.cover_image_url" class="aspect-video bg-[#1A1A24] flex items-center justify-center overflow-hidden rounded-lg mb-3">
									<img :src="model.cover_image_url" class="object-cover w-full h-full group-hover:scale-105 transition-transform duration-500" :alt="model.name" />
								</div>
								<div class="font-sans font-semibold text-sm mb-1 text-[#FAF8F5] truncate" :title="model.name">{{ model.name }}</div>
								<div class="text-[11px] font-sans text-[#FAF8F5]/50 line-clamp-2 mb-3 mt-1" :title="model.description">{{ model.description }}</div>
								<a :href="model.url" target="_blank" class="mt-auto text-[10px] font-mono uppercase tracking-widest text-[#C9A84C] hover:text-[#FAF8F5] transition-colors">Open Model ↗</a>
							</div>
						</div>
					</div>
				</div>
				...
				<div v-else class="flex items-start gap-3 max-w-[85%]">
					<div class="bg-[#2A2A35] border border-[#2A2A35] text-[#FAF8F5] rounded-2xl rounded-tr-sm px-5 py-3 text-sm leading-relaxed whitespace-pre-wrap shadow-sm font-sans mt-1">
						{{ m.text }}
					</div>
					<div class="w-8 h-8 rounded-full bg-[#C9A84C] flex items-center justify-center text-xs font-serif font-bold italic text-[#0D0D12] flex-shrink-0 mt-1">U</div>
				</div>
			</div>

			<!-- Typing indicator -->
			<div v-if="isResponding" class="flex items-start gap-3 mb-2">
			    <div class="w-8 h-8 rounded-full bg-[#2A2A35] border border-[#C9A84C]/30 flex items-center justify-center text-xs font-serif font-bold italic text-[#C9A84C] shadow-[0_0_10px_rgba(201,168,76,0.1)] flex-shrink-0 animate-pulse">A</div>
		...
		<div class="border-t border-[#2A2A35] p-5 bg-[#0D0D12]">
			<div class="bg-[#1A1A24] border border-[#2A2A35] rounded-3xl p-3 shadow-inner focus-within:border-[#C9A84C]/50 transition-colors">
				<textarea
					ref="composer"
					v-model="draft"
				...
				<div class="flex items-center justify-between mt-3 px-1">
					<div class="flex items-center gap-2 hidde">
						<button
							@click="insertAtCursor('{lora:example:1.0}')"
			...
			<div class="flex items-center justify-between mt-4 px-2">
				...
				<div class="flex items-center gap-3">
					<button @click="regenerateLast" class="text-[10px] font-mono uppercase tracking-widest px-3 py-1.5 rounded-lg text-[#FAF8F5]/50 hover:text-[#C9A84C] transition-colors">
						Regenerate
					</button>
					<button @click="clearChat" class="text-[10px] font-mono uppercase tracking-widest px-3 py-1.5 rounded-lg text-[#FAF8F5]/50 hover:text-red-400 transition-colors">
						Purge
					</button>
				</div>
			</div>
